...
class QuestionModelTests(TestCase):
    ...
    def test_was_published_recently_with_future_question(self):
        """
            was_published_recntely returns false for questions whose pub_daate is in the future
        """
        time = timezone.now() + datetime.timedelta(days=30)
        future_question = Question(question_text="¿Quien es el mejor Course Directos de PLatzi?", pub_date = time)
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_two_past_questions(self):
        ...
        past_question2 = create_question(question_text="Past questions", days=-40)
        response = self.client.get(reverse("polls:index"))
        self.assertQuerysetEqual(
            response.context["latest_question_list"],
...
class QuestionDetailViewTests(TestCase):
    def test_future_question(self):
        """
            The detail view of a quetion with a pub_date in the future
